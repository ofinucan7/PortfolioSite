...
const Skills = ({ focus_skills, all_skills, change_text_color } ) => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-8 py-8 sm:py-12 lg:py-20 text-[#F1EFEC] mt-12 sm:mt-12 md:mt-16 lg:-mt-2 xl:-mt-8 2xl:mt-2">
          ...
          <div className="bg-[#D4C9BE] p-4 sm:p-6 lg:p-8 rounded-lg sm:rounded-xl border-4 sm:border-[8px] border-[#123458] max-w-sm sm:max-w-md lg:max-w-lg w-full text-[#030303] relative max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            {selectedSkill.projectsUsed?.length > 0 && (
              <div className="mb-4 sm:mb-6">
                ...
                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-xs sm:text-sm text-[#030303] list-disc list-inside">
                  {selectedSkill.projectsUsed.map((project, idx) => (
                    <li key={idx} className="list-item break-words"> 
                      {project.name}{" "} 
                      <a href={project.link} target="_blank" rel="noopener noreferrer" className="text-[#123458] hover:underline font-medium ml-1 break-all"> 
                        | Go to Project 
                      </a> 
                    </li>
                  ))}
